...
**Truncation is data, not a log line.** `MAX_ENTRY_PAGES` (20, ~300 entries) bounds a walk, and a bounded walk returns `EventSnapshot { complete: false }` (new on `CalendarProvider`, defaulting to `true` for server-filtered providers). `sync_provider_source` runs `remove_orphaned_provider_events` — which deletes local events *and* `UPDATE bookings SET status='cancelled'` for their UIDs — only against a complete snapshot. calrs' own write-back entries carry the booking UID, so without that gate a schedule over the cap would cancel confirmed bookings. The same function now also skips fetching events for `is_busy = 0` calendars: they block nothing, and on a Basecamp account with many projects that is what keeps an on-demand sync from walking every schedule on a public slot page. Recurring entries are read as single events: Basecamp models recurrence with a `recurrence_schedule` object rather than an RRULE, and a wrong translation would silently mis-block availability, so it is documented rather than approximated.
